function float_vacc_core_init(blk, varargin)

defaults = {};
%disp('hi from vacc_core_init')
if same_state(blk, 'defaults', defaults, varargin{:}), return, end
check_mask_type(blk, 'float_vacc_core');
...
delay = veclen - add_latency - mux_latency - conv_latency;
bram_latency = eval(bram_latency);
delay_bits = max(nextpow2(delay), 2);
%num2str(delay - bram_latency - 1)

set_param([blk, '/Convert'], 'latency', num2str(conv_latency));
set_param([blk, '/Delay'], 'latency', num2str(conv_latency));
set_param([blk, '/Delay1'], 'latency', num2str(conv_latency));
